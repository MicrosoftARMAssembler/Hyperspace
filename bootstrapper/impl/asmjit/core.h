...
//!     `x86::Compiler::call()` instead of `call()`. The reason behind this is to remove the confusion between a
//!     `call` instruction and AsmJit's `call()` intrinsic, which is now `call()`.
//!
//!   - Creating new nodes also changed. Now the preferred way of invoking a function is to call
//!     `x86::Compiler::call()` where the first argument is `InvokeNode**`. The function now returns an error and
...
//! }
//!
//! // Calling a function (Compiler) changed - use call() instead of call().
//! void functionInvocation(x86::Compiler& cc) {
//!   InvokeNode* invokeNode;
//!   cc.call(&invokeNode, targetOperand, FuncSignature::build<...>(...));
//! }
//! ```
...
//!     printf("Could not flatten the code: %s\n", DebugUtils::errorAsString(err));
//!     exit(1);
//!   }
//!
...
//!     printf("Could not resolve label links: %s\n", DebugUtils::errorAsString(err));
//!     exit(1);
//!   }
//!
